...
<TABLE border=0 cellpadding="0" cellspacing=1 width="600">
	...
	<TD align="right"><?if($user_cookie == $crow["forum_owner"]){?><A href="javascript:delIt()">delete forum <img align="absmiddle" src="./images/icons/trash.gif" width="16" height="16" alt="Delete this forum" border="0"></a><?}?></td>
	</tr>
</TABLE>
...
<input type="hidden" name="forum_unique_update" value="<?echo uniqid("");?>">
<input type="hidden" name="forum_id" value="<?echo $forum_id;?>">
<TR bgcolor="#878676" height="20" style="border: outset #eeeeee 2px;">
	<TD valign="top" colspan=2><b><i><?if($forum_id == 0){echo "Add";}else{echo "Edit";}?> forum </i></b></td>
...
<tr>
	<TD colspan=2>
			<TABLE border=0 cellpadding=1 cellspacing=1 bgcolor="black" width="100%">
					<tr bgcolor="#f4efe3"><TD align="right" width="100">Forum Name: </td><TD><input type="text" class="text" size=25 name="forum_name" value="<?echo @$crow["forum_name"];?>" maxlength="50" style="width:200px;"></td></tr>
	...
	<TD valign="top">
			<TABLE border=0 cellpadding=1 cellspacing=1 bgcolor="silver">
					...
					<option value=0 <?if(intval($crow["forum_owner"])==0) echo " selected ";?>>
					<? while($row = mysql_fetch_array($rcu)){
				  ...
				  <tr bgcolor="#eeeeee" height=20><TD align="right" nowrap valign="top">Forum Status:</td>
					<TD valign="top">
					<input type="radio" value="-1" <?if($status ==-1)echo " checked";?> name="forum_status">open for posting<br>
					<input type="radio" value="1" <?if($status ==1)echo " checked";?> name="forum_status">read-only<br>
					<input type="radio" value="0" <?if($status ==0)echo " checked";?> name="forum_status">closed </td></tr>
					
				  <tr bgcolor="#eeeeee" height=20><TD align="right" nowrap>Forum Moderated:</td><TD><input type="checkbox" name="forum_moderated" value=1 <?if(intval($crow["forum_moderated"])<>0)echo " checked "?>></td></tr>
					
					<tr bgcolor="#eeeeee" height=20><TD align="right">Created On</td><TD><?echo @$crow["forum_create_date"];?></td></tr>
					<tr bgcolor="#eeeeee" height=20><TD align="right">Last Post:</td><TD><?echo @$crow["forum_last_date"];?></td></tr>
					<tr bgcolor="#eeeeee" height=20><TD align="right" nowrap>Message Count:</td><TD><?echo @$crow["forum_message_count"];?></td></tr>
			</table>
	...
	<TD valign="top">
		<TABLE border=0 cellpadding=1 cellspacing=1 bgcolor="silver">
			<tr bgcolor="#eeeeee" height=170>
				...
				<textarea class="textarea" name="forum_description" style="height:150px;"><?echo @$crow["forum_description"];?></textarea></TD>
			</TR>
		</TABLE>
	</td>
</tr>
...
<TD align="right"><?if($user_cookie == $crow["forum_owner"] || $forum_id ==0){?><input type="button" value="submit" class=button onClick="submitIt()"><?}?></td></tr>
</form>
</TABLE>
