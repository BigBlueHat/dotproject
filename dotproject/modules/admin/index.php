		...
		<TD valign="top" colspan=2>
			<TABLE width="100%" cellpadding="1" cellspacing=1 >
				...
				<?while($row = mysql_fetch_array($urow)){?>
					<?if(intval(@$row["permission_user"]) !=0){echo "<TR bgcolor=#f4efe3>";} else { echo "<TR bgcolor=#eeeeee>";}?>
					<TD width="60" class="smallNorm" align="right"><A href="./index.php?m=admin&a=addedituser&user_id=<?echo $row["user_id"];?>"><span  class="smallNorm">edit</a> |</span> <A href="javascript:delMe(<?echo $row["user_id"];?>, '<?echo $row["user_first_name"] . " " . $row["user_last_name"];?>')"><span  class="smallNorm">del</span></a>&nbsp; </td>
					<TD><?echo $row["user_username"];?></td>
					<TD><?echo $row["user_last_name"];?>, <?echo $row["user_first_name"];?></td>
					<TD><A href="mailto:<?echo $row["user_email"];?>"><?echo $row["user_email"];?></a></td>
					<TD><?if(intval(@$row["permission_user"]) !=0){echo "Yes";} else { echo "No";}?> </td>
					<TD><input type="button" class=button value="permissions" onClick="javascript:window.location= './index.php?m=admin&a=permissions&user_id=<?echo $row["user_id"];?>';"></td>
