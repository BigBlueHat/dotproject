<?php /* CLASSES $Id: date.class.php,v 1.38 2007/07/26 20:50:39 merlinyoda Exp $ */
/**
* @package dotproject
* @subpackage utilites
* @file date.class.php
...
class DP_Date {
	 ...
	 */
	public function compare($d1, $d2 = null, $convertTZ=false)
	{
		if ($d2 === null) {
			$d2 = new self($d1);
			$d1 = new self($this);
	...
	{
	  $working_days = DP_Config::getConfig('cal_working_days');
	...
	public function next_working_day( $preserveHours = false ) {
		$do = $this;
		$end = intval(DP_Config::getConfig('cal_day_end'));
		$start = intval(DP_Config::getConfig('cal_day_start'));
	...
	public function addDuration( $duration = '8', $durationType ='1') {
		...
		if ($durationType == '24') { // duration type is 24, full days, we're finished very quickly
			...
			$cal_day_start = intval(DP_Config::getConfig( 'cal_day_start' ));
			$cal_day_end = intval(DP_Config::getConfig( 'cal_day_end' ));
			$dwh = intval(DP_Config::getConfig( 'daily_working_hours' ));
	...
	public function format($format = null, $convert = null)
	{
		$local_date = new self($this);
	...
	public function workingDaysInSpan($e) 
	{
		...
		$start = $e->before($this) ? new self($e) : new self($this);
		for ( $i = 0 ; $i <= $days ; $i++ ){
	...
	public function getWeekdayAbbrname($day = false) 
	{
		if ($day === false) {
			$day = $this->format('%w');
		}
	...
	public function getCalendarMonth($date)
	{
		$ts = $date->getTime();
		$daysInMonth = date('t', $ts);
		$monthDay = new self($date);
		$monthDay->setDay(1);
