 ...
 */
	public function __construct( $table, $key )
	{
		...
		$this->_prefix = DP_Config::getConfig('dbprefix', '');
		$this->_query = new DP_Query;
	} // }}} constructor
 ...
 */
	function load( $oid=null , $strip = true)
	{
		...
		$obj = $this->_query->loadObject( $this, false, $strip );
 ...
 */
	function loadAll($order = null, $where = null)
	{
		...
		return $this->_query->loadHashList($this->_tbl_key);
	}

/**
 *	Return a DP_Query object seeded with the table name.
 ...
 */
	function store( $updateNulls = true ) {
		...
		$k = $this->_tbl_key;
		if( $this->$k ) {
			$ret = $this->_query->updateObject( $this->_tbl, $this, $this->_tbl_key, $updateNulls );
			$action = 'update';
		} else {
			$ret = $this->_query->insertObject( $this->_tbl, $this, $this->_tbl_key );
			$action = 'add';
		}
		...
		return ((!$ret)?(get_class( $this ) . "::store failed <br />" . $this->_query->error()):NULL) ;
	}
 ...
 */
	function canDelete( &$msg, $oid=null, $joins=null )
	{
		...
		if (is_array( $joins )) {
			...
			$this->_query->clear();
			$this->_query->addTable($this->_tbl);
			$this->_query->addWhere("$k = '".$this->$k."'");
			$this->_query->addGroup($k);
			foreach( $joins as $table ) {
				$this->_query->addQuery("COUNT(DISTINCT {$table['idfield']}) AS {$table['idfield']}");
				$this->_query->addJoin($table['name'], $table['name'], "{$table['joinfield']} = $k");
			}
			$obj = null;
			if (!$this->_query->loadObject( $sql, $obj )) {
				$msg = db_error();
				return false;
			}
			$msg = array();
 ...
 */
	function delete( $oid=null )
	{
		...
		$this->_query->clear();
		$this->_query->setDelete($this->_tbl);
		$this->_query->addWhere("$this->_tbl_key = '".$this->$k."'");
		$result = ((!$this->_query->exec())?$this->_query->error():null);
		if (!$result) {
 ...
 */
	function getDeniedRecords( $uid )
	{
		...
		$perms =& DP_AppUI::getInstance()->acl();
