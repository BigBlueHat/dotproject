...
class DP_Localisation
{
	...
	{
		if ( isset($_SESSION['LANGUAGES']) && count($_SESSION['LANGUAGES'])) {
			$LANGUAGES =& $_SESSION['LANGUAGES'];
		} else {
			$LANGUAGES = array();
			$langs = DP_AppUI::getInstance()->readDirs('locales');
			foreach ($langs as $lang) {
				if (file_exists(DP_BASE_CODE."/locales/$lang/lang.php")) {
					include_once DP_BASE_CODE."/locales/$lang/lang.php";
				}
			}
 ...
 /**
	...
	*/
	function setUserLocale($loc = '', $set = true)
	{
		...
		if (! $loc) {
			$loc = @$AppUI->user_prefs['LOCALE'] ? $AppUI->user_prefs['LOCALE'] : $AppUI->getConfig('host_locale');
		}
		...
		if ($set) {
			...
			@include_once(DP_BASE_CODE . '/locales/'.$this->locale.'/locales.php');
			$this->loadTranslation();
			@include_once(DP_BASE_CODE . '/locales/core.php');
		} else {
			return $user_lang;
		}
	}
	...
	function loadTranslation($locale = null)
	{
		...
		$translations = file(DP_BASE_CODE . '/locales/'.$locale.'/common.inc');
		...
		if (file_exists(DP_BASE_CODE . "/modules/$m/locales/$locale.inc")) {
			$translations = array_merge($translations, file(DP_BASE_CODE . "/modules/$m/locales/$locale.inc"));
		} elseif (file_exists(DP_BASE_CODE . "/locales/$locale/$m.inc")) {
			$translations = array_merge($translations, file(DP_BASE_CODE . "/locales/$locale/$m.inc"));
		}
		...
		{
			if (file_exists(DP_BASE_CODE . "/modules/$extra_module/locales/$locale.inc")) {
	    	$translations = array_merge($translations, file(DP_BASE_CODE . "/modules/$extra_module/locales/$locale.inc"));
			} elseif (file_exists(DP_BASE_CODE . "/locales/$locale/$extra_module.inc")) {
				$translations = array_merge($translations, file(DP_BASE_CODE . "/locales/$locale/$extra_module.inc"));
			}
		}
		}
		...
		switch ($m) {
		case 'departments':
			$translations = array_merge($translations, file(DP_BASE_CODE . "/locales/$locale/companies.inc" ));
			break;
		case 'system':
			$translations = array_merge($translations, file(DP_BASE_CODE . '/locales/'.DP_Config::getConfig('host_locale') . '/styles.inc'));
			break;
		}
