<?php // $Id: event_queue.class.php,v 1.13 2007/03/20 02:10:38 mosen Exp $
...
class DP_EventQueue {
	...
	public static $table = 'event_queue'; /**< Table to use for events, always 'event_queue' */
	public $update_list = array(); /**< List of events to update */
	public $delete_list = array(); /**< List of events to delete */
	public $event_count = 0; /**< Counter containing number of events to execute */
	...
	public function _scan()
	{
		...
		$rows = $q->loadList();
		foreach ($rows as $row) {
			if (($res = $this->execute($row)) !== false ) {
				$this->update_event($row, $res);
				$this->event_count++;
			}
		}
		$q->clear();
