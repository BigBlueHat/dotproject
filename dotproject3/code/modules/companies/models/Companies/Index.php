...
class Companies_Index extends DP_List_Dynamic {
	...
	public function __construct() {
		...
		$select->joinLeft(array('p'=>'projects'),
			'company_id = p.project_company AND p.project_status != 7',
			array('COUNT(distinct p.project_id) as company_projects_active')
		);
		
		
		$select->joinLeft(array('p2'=>'projects'),
			'company_id = p2.project_company AND p.project_status = 7',
			array('COUNT(distinct p2.project_id)as company_projects_inactive')
		);

		$select->join(array('u'=>'users'),
			'company_owner = u.user_id',
			Array()
		);

		$select->join(array('con'=>'contacts'),
			'u.user_contact = con.contact_id',
			Array('con.contact_first_name','con.contact_last_name')
		);
